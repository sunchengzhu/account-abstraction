...
import {
  GaslessEntryPoint as EntryPoint
} from '../typechain'
import { GaslessUserOperation } from './GaslessUserOperation'
...
export function packUserOp (op: GaslessUserOperation, forSignature = false): string {
  if (forSignature) {
    // lighter signature scheme (must match UserOperation#pack): do encode a zero-length signature, but strip afterwards the appended zero-length value
    const userOpType = {
      components: [
        { type: 'address', name: 'callContract' },
  ...
  const typevalues = [
    { type: 'address', val: op.callContract },
...
export function packUserOp1 (op: GaslessUserOperation): string {
...
export function getRequestId (op: GaslessUserOperation, entryPoint: string, chainId: number): string {
...
export const DefaultsForUserOp: GaslessUserOperation = {
  callContract: AddressZero,
  callData: '0x',
  callGasLimit: 0,
...
export function signUserOp (op: GaslessUserOperation, signer: Wallet, entryPoint: string, chainId: number): GaslessUserOperation {
...
export function fillUserOpDefaults (op: Partial<GaslessUserOperation>, defaults = DefaultsForUserOp): GaslessUserOperation {
...
export async function fillUserOp (op: Partial<GaslessUserOperation>, entryPoint?: EntryPoint): Promise<GaslessUserOperation> {
  const op1 = { ...op }
...
export async function fillAndSign (op: Partial<GaslessUserOperation>, signer: Wallet | Signer, entryPoint?: EntryPoint): Promise<GaslessUserOperation> {
